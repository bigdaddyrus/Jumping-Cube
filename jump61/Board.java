...
abstract class Board extends Observable {
    ...
    /** Returns if the square at I, J is full. */
    abstract boolean isFull(int i, int j);
    ...
    boolean isLegal(Side player, int n) {
        return player.equals(get(n).getSide())
            || get(n).getSide() == WHITE;
    }

    /** Returns true iff PLAYER is allowed to move at this point. */
    boolean isLegal(Side player) {
        if (numOfSide(player.opposite()) == (size() * size())) {
            return false;
        }
        return true;
    }
