...
class Game extends Observable {
    ...
    void makeMove(int r, int c) {
        ...
        } else if (_board.isLegal(_board.whoseMove(), r, c)) {
    ...
    void makeMove(int n) {
        ...
        int row = _board.row(n);
        int col = _board.col(n);
        if (row > _board.size()
                || col > _board.size()) {
            reportError("move %d %d out of bounds", row, col);
        } else if (_board.isLegal(_board.whoseMove(), n)) {
    ...
    /** Report a move by PLAYER using N. */
    void reportMove(Side player, int n) {
        message("%s moves %d %d.%n", player.toCapitalizedString(),
    ...
    private void setSpots(int r, int c, int spots, String color) {
        if (spots == 0) {
            ...
            if (spots > _board.neighbors(r, c)
                || spots < 0) {
                reportError("invalid request to put %d on square %d %d",
                                        spots, r, c);
            } else {
                if (color.equals("b")) {
                    Side colo  = BLUE;
                    _board.set(r, c, spots, colo);
                    _playing = false;
                }
                if (color.equals("r")) {
                    Side colo = RED;
    ...
    private void executeCommand(String cmnd) {
        try {
            ...
            reportError("invalid syntax with '%s' command", cmnd);
        }
    }
    ...
    /** The board on which I record all moves. Modified */
