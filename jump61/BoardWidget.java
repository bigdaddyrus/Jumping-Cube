...
class BoardWidget extends Pad {
    ...
    private static final Color
        ...
        WINNING_GOLD = new Color(239, 172, 31);

    /** The Writing Font. */
    static final Font WINNING_FONT = new Font("SansSerif", 1, 20);
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        if (winner != null) {
            g.setFont(WINNING_FONT);
            FontMetrics metrics = g.getFontMetrics();
            String winnerString = winner.toCapitalizedString();
            g.setColor(WINNING_GOLD);
            g.drawString(winnerString + " Wins!",
                         (_side
                          - metrics.stringWidth(winnerString + " Wins!")) / 2,
                         (2 * _side + metrics.getMaxAscent()) / 4);
        }
    }
    /** fill the RECTANGLE on G at R, C. */
    private void fillRects(Graphics2D g, int r, int c) {
        ...
        g.fillRect((r - 1) * SQUARE_SEP + SEPARATOR_SIZE,
                    (c - 1) * SQUARE_SEP + SEPARATOR_SIZE,
                    SQUARE_SIZE, SQUARE_SIZE);
    }
    /** Color and display the spots on the square at row R and column C
     *  on G.  (Used by paintComponent). Need edit*/
    private void displaySpots(Graphics2D g, int r, int c) {
        if (_board.get(r, c).getSpots() == 1) {
            spot(g, SQUARE_SEP * r - SQUARE_SEP / 2,
                    SQUARE_SEP * c - SQUARE_SEP / 2);
        }
        if (_board.get(r, c).getSpots() == 2) {
            spot(g, SQUARE_SEP * r - SQUARE_SEP / 4,
                    SQUARE_SEP * c - SQUARE_SEP * 3 / 4);
            spot(g, SQUARE_SEP * r - SQUARE_SEP * 3 / 4,
                    SQUARE_SEP * c - SQUARE_SEP / 4);
        }
        if (_board.get(r, c).getSpots() == 3) {
            spot(g, SQUARE_SEP * r - SQUARE_SEP / 4,
                    SQUARE_SEP * c - SQUARE_SEP * 3 / 4);
            spot(g, SQUARE_SEP * r - SQUARE_SEP / 2,
                    SQUARE_SEP * c - SQUARE_SEP / 2);
    ...
    private void spot(Graphics2D g, int x, int y) {
        ...
        g.fillOval(x - SPOT_DIM / 2, y - SPOT_DIM / 2,
                SPOT_DIM, SPOT_DIM);
    }

    /** Respond to the mouse click depicted by EVENT. Need edit */
